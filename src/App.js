...
import { Camera, FileUp, UtensilsCrossed, ChefHat, Heart, LoaderCircle, AlertTriangle, X, Clock } from 'lucide-react';

// Hilfsfunktion zum Konvertieren einer Datei in eine Base64-Zeichenkette
const toBase64 = file => new Promise((resolve, reject) => {
    const reader = new FileReader();
    reader.readAsDataURL(file);
    reader.onload = () => resolve(reader.result.split(',')[1]); // Nur den Base64-Teil zurückgeben
...
export default function App() {
    ...
    const getIngredientsFromImage = useCallback(async (file) => {
        if (!file) return;
        ...
        // Erzeugt eine temporäre URL für die Anzeige des Bildes
